...
describe "OracleEnhancedAdapter structure dump" do
  ...
  describe "structure dump" do
    ...
    it "should dump virtual columns" do
      skip "Not supported in this database version" unless @conn.database_version.first >= 11
      @conn.execute <<~SQL
        CREATE TABLE bars (
  ...
  describe "structure_dump for identity columns" do
    before do
      skip "requires Oracle 12.1+" unless @conn.supports_identity_columns?
    end

    after do
      @conn.drop_table :test_identity_dump, if_exists: true
    end

    def test_identity_dump_block(dump)
      dump[/CREATE TABLE "TEST_IDENTITY_DUMP" \(.*?\n\)/m]
    end

    it "emits GENERATED BY DEFAULT AS IDENTITY for identity columns" do
      @conn.create_table :test_identity_dump, force: true, identity: true do |t|
        t.string :name
      end
      block = test_identity_dump_block(@conn.structure_dump)
      expect(block).not_to be_nil
      expect(block).to match(/"ID" NUMBER\(\d+(?:,\d+)?\)\s+GENERATED BY DEFAULT AS IDENTITY/)
    end

    it "omits the ISEQ\\$\\$_ NEXTVAL data_default for identity columns" do
      @conn.create_table :test_identity_dump, force: true, identity: true do |t|
        t.string :name
      end
      block = test_identity_dump_block(@conn.structure_dump)
