    ...
    module OracleEnhanced
      module DatabaseStatements
        ...
        private
          ...
          # like +code: "ABC"+) the bind is present (or the literal is interpolated
          # into the column list under prepared_statements: false) and there is
          # nothing to read back; appending RETURNING with an Integer-typed
          # out-bind would raise ORA-01722 against a non-numeric column.
          def sql_for_insert(sql, pk, binds, _returning) # :nodoc:
            if (pk.is_a?(Symbol) || pk.is_a?(String)) && !insert_supplies_pk?(sql, binds, pk.to_s)
          ...
          # True if the caller already provided the primary key value for the
          # INSERT, either as a bind (prepared_statements: true) or interpolated
          # into the column list (prepared_statements: false). The all-DEFAULT
          # form generated by empty_insert_statement_value (auto-generated PK)
          # is treated as not-supplied so the caller still gets RETURNING for
          # the database-generated value.
          def insert_supplies_pk?(sql, binds, pk_name)
            return true if binds.any? { |bind| bind.name == pk_name }
            match = sql.match(/\AINSERT\s+INTO\s+\S+\s*\(([^)]*)\)\s*VALUES\s*\(([^)]*)\)/i)
            return false unless match
            return false if match[2].strip.casecmp?("DEFAULT")
            match[1].scan(/[A-Za-z_][\w$#]*/).any? { |col| col.casecmp?(pk_name) }
          end
