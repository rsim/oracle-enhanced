  ...
  module ConnectionAdapters #:nodoc:
    ...
    class OracleEnhancedAdapter < AbstractAdapter
      ...
      def next_sequence_value(sequence_name)
        ...
        if sequence_name.include?('@')
          sequence_name, db_link = sequence_name.split('@')
          db_link = "@#{db_link}"
        else
          db_link = nil
        end
        select_one("SELECT #{quote_table_name(sequence_name)}.NEXTVAL#{db_link} id FROM dual#{db_link}")['id']
      end
      ...
      def default_sequence_name(table_name, primary_key = nil)
        ...
        # remove database link if present
        if table_name.include?('@')
          table_name, db_link = table_name.split('@')
          db_link = "@#{db_link}"
        else
          db_link = nil
        end
        ...
        "#{table_name[0,IDENTIFIER_MAX_LENGTH-4]}_seq#{db_link}"
      end
